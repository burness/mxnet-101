...
def block17(net, input_data_shape, scale=1.0, with_act=True, act_type='relu', mirror_attr={}):
    assert len(input_data_shape) == 4, 'input_data_shape should be len of 4, your \
                            input_data_shape is len of %d'%len(input_data_shape)
    _, out_shape, _ = net.infer_shape(data=input_data_shape)
...
def block8(net, input_data_shape, scale=1.0, with_act=True, act_type='relu', mirror_attr={}):
    assert len(input_data_shape) == 4, 'input_data_shape should be len of 4, your \
                            input_data_shape is len of %d'%len(input_data_shape)
    _, out_shape,_ = net.infer_shape(data=input_data_shape)
    tower_conv = ConvFactory(net, 192, (1, 1))
...
def get_symbol(num_classes=1000,input_data_shape=(128,3,299,299)):
    ...
    _,out_shape,_ = tower_5b_out.infer_shape(data=input_data_shape)
    print out_shape
    net = repeat(tower_5b_out, 10, block35, scale=0.17, input_data_shape=input_data_shape)
    ...
    _,out_shape,_ = net.infer_shape(data=input_data_shape)
    print out_shape
    net = repeat(net, 20, block17, scale=0.1, input_data_shape=input_data_shape)
    _,out_shape,_ = net.infer_shape(data=input_data_shape)
    print out_shape
    tower_conv = ConvFactory(net, 256, (1,1))
    tower_conv0_1 = ConvFactory(tower_conv, 384, (3,3), stride=(2,2))
    _, out_shape1,_ = tower_conv0_1.infer_shape(data=input_data_shape)
    print out_shape1
    tower_conv1 = ConvFactory(net, 256, (1,1))
    tower_conv1_1 = ConvFactory(tower_conv1, 288, (3,3), stride=(2,2))
    _, out_shape1,_ = tower_conv1_1.infer_shape(data=input_data_shape)
    print out_shape1
    ...
    _, out_shape1,_ = tower_conv2_2.infer_shape(data=input_data_shape)    
    print out_shape1
    tower_pool = mx.symbol.Pooling(net, kernel=(3,3), stride=(2,2), pool_type='max')
    _, out_shape1, _ = tower_pool.infer_shape(data=input_data_shape)
    print out_shape1
    net = mx.symbol.Concat(*[tower_conv0_1, tower_conv1_1, tower_conv2_2, tower_pool])
    _, out_shape1,_ = net.infer_shape(data=input_data_shape)
    print 'nima',out_shape1

    net = repeat(net, 9, block8, scale=0.2, input_data_shape=input_data_shape)
    net = block8(net, with_act=False, input_data_shape=input_data_shape)
